...
public class ScriptsFragment extends Fragment {
    ...
    private void loadScripts() {
        ...
        try {
            ...
            if (prefsDir.exists()) {
                Process chmodPrefs = Runtime.getRuntime().exec("chmod 755 " + prefsDir.getAbsolutePath());
                chmodPrefs.waitFor();
                
                // Make the script_contents.xml file readable if it exists
                File prefsFile = new File(prefsDir, "script_contents.xml");
                if (prefsFile.exists()) {
                    Process chmodPrefsFile = Runtime.getRuntime().exec("chmod 644 " + prefsFile.getAbsolutePath());
